...
export default function ChatWindow({ messages, isStreaming, streamingText, sendMessage, onOpenSidebar }) {
  const { user } = useAuth();
  ...
  return (
    <main className="pt-16 lg:ml-64 h-screen flex flex-col bg-surface dark:bg-[#0f1117] transition-colors">
      {!hasMessages ? (
        /* ── Empty State ── */
        <div className="flex-1 flex flex-col items-center justify-center px-4 sm:px-6 pb-36 pt-8 sm:pt-10">
          <button
            onClick={onOpenSidebar}
            className="lg:hidden mb-6 self-start inline-flex items-center gap-2 rounded-xl border border-slate-200 dark:border-[#2a2f4a] bg-white dark:bg-[#1a1d27] px-4 py-2 text-sm font-semibold text-slate-700 dark:text-slate-200"
          >
        ...
        /* ── Messages ── */
        <div className="flex-1 overflow-y-auto px-4 sm:px-6 pb-40 pt-4 sm:pt-6 scrollbar-hide">
          <div className="max-w-3xl mx-auto space-y-6">
            <div className="lg:hidden">
              <button
                onClick={onOpenSidebar}
                className="inline-flex items-center gap-2 rounded-xl border border-slate-200 dark:border-[#2a2f4a] bg-white dark:bg-[#1a1d27] px-4 py-2 text-sm font-semibold text-slate-700 dark:text-slate-200"
              >
                <span className="material-symbols-outlined text-[18px]">menu</span>
                Open Menu
              </button>
            ...
            {isStreaming && (streamingText ? (
              <div className="flex justify-start items-start gap-3 animate-fade-in">
                ...
                <div className="bg-white dark:bg-[#1a1d27] border border-slate-200 dark:border-[#2a2f4a] px-6 py-5 rounded-2xl rounded-tl-sm shadow-sm max-w-[92%] sm:max-w-[85%] relative overflow-hidden">
      ...
      {/* ── Input Bar ── */}
      <div className="fixed bottom-0 left-0 right-0 lg:left-64 p-4 sm:p-6 bg-gradient-to-t from-surface dark:from-[#0f1117] via-surface/95 dark:via-[#0f1117]/95 to-transparent pointer-events-none z-10">
        <div className="max-w-3xl mx-auto w-full pointer-events-auto">
          <div className="relative bg-white dark:bg-[#1a1d27] border border-slate-200 dark:border-[#2a2f4a]
            rounded-2xl shadow-xl shadow-slate-200/60 dark:shadow-black/40 p-2 flex items-end gap-1.5 sm:gap-2
          ...
          <div className="flex flex-col sm:flex-row justify-center items-center gap-2 sm:gap-6 mt-3">
