...
const INITIAL_DOCS = [
  { id: 1, name: 'Q4_Retail_Safety_Protocol_v2.pdf', size: '7.2 MB', type: 'PDF', pages: 42, date: 'Oct 24, 2023, 11:02 AM', status: 'indexing' },
  { id: 2, name: 'Emergency_Evacuation_Guidelines.docx', size: '1.4 MB', type: 'DOCX', pages: 8, date: 'Oct 23, 2023, 04:45 PM', status: 'success' },
  { id: 3, name: 'Inventory_Procurement_Workflow.pdf', size: '12.8 MB', type: 'PDF', pages: 115, date: 'Oct 22, 2023, 09:12 AM', status: 'success' },
  { id: 4, name: 'Cloud_Migration_Framework.json', size: '0.8 MB', type: 'JSON', pages: null, date: 'Oct 21, 2023, 02:20 PM', status: 'indexing' },
  { id: 5, name: 'HR_Onboarding_Handbook_2024.pdf', size: '4.1 MB', type: 'PDF', pages: 68, date: 'Oct 20, 2023, 10:00 AM', status: 'success' },
];
...
export default function AdminDashboard() {
  ...
  const handleDelete = (id) => { setDocs(prev => prev.filter(d => d.id !== id)); showToast('Document deleted.'); };
    ...
    <main className="ml-64 pt-24 pb-16 px-8 bg-surface dark:bg-[#0f1117] min-h-screen transition-colors">
      ...
      <div className="max-w-[1440px] mx-auto">
              ...
              <input type="file" className="hidden" accept=".pdf,.docx,.xlsx,.json"
                onChange={e => {
                  if (e.target.files[0]) {
                    ...
                    setDocs(prev => [{ id: Date.now(), name: f.name, size: (f.size/1024/1024).toFixed(1)+' MB', type: f.name.split('.').pop().toUpperCase(), pages: null, date: new Date().toLocaleString(), status: 'indexing' }, ...prev]);
                    showToast(`"${f.name}" uploaded and indexing started.`);
                  }
                }}
              />
            </label>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-10">
          {[
            { label: 'TOTAL DOCUMENTS', value: docs.length.toLocaleString(), sub: '+12 this week', subColor: 'text-secondary' },
            { label: 'ACTIVE CHUNKS', value: '45.2k', sub: 'Vector Optimized', subColor: 'text-slate-400 dark:text-slate-500' },
            { label: 'STORAGE USED', value: '4.2 GB', sub: 'of 10 GB', subColor: 'text-slate-400 dark:text-slate-500' },
                ...
                {filtered.map(doc => (
                  <tr key={doc.id} className="hover:bg-slate-50 dark:hover:bg-[#161820] transition-colors">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${doc.status === 'indexing' ? 'bg-secondary-container/20 dark:bg-secondary/15 text-secondary' : 'bg-slate-100 dark:bg-[#21253a] text-slate-500 dark:text-slate-400'}`}>
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-1">
                        <button className="p-2 hover:bg-slate-100 dark:hover:bg-[#21253a] rounded-lg text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300 transition-colors">
                          <span className="material-symbols-outlined text-[18px]">visibility</span>
                        </button>
                        <button onClick={() => handleDelete(doc.id)} className="p-2 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg text-slate-400 dark:text-slate-500 hover:text-error transition-colors">
        ...
        <div className="mt-10 grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 bg-white dark:bg-[#1a1d27] rounded-2xl border border-slate-200 dark:border-[#2a2f4a] p-6 shadow-sm">
            ...
            <div className="space-y-5">
              {[
                { dot: 'bg-secondary', title: 'Vector Database Optimization', time: '10 mins ago', desc: 'Successfully re-balanced 12,400 embeddings across primary and secondary nodes.' },
                { dot: 'bg-amber-500', title: 'New SOP Ingestion', time: '1 hour ago', desc: 'Retail_Safety_Protocol_v2.pdf started chunking. Estimated time: 4 mins.' },
                { dot: 'bg-secondary', title: 'Auto Re-index Complete', time: '3 hours ago', desc: 'Scheduled daily re-index completed with 0 errors across 8,420 documents.' },
              ].map(a => (
          ...
          <div className="bg-primary-container text-white rounded-2xl p-6 shadow-xl relative overflow-hidden">
            ...
            <div className="relative z-10 h-full flex flex-col">
              ...
              <p className="text-sm text-slate-300 mb-6 leading-relaxed">Your knowledge base covers 94% of historical user queries. Consider adding "Internal IT Policy" documents to bridge the gap.</p>
